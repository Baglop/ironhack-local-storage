...
import { ADD_CATEGORY, ADD_TASK, CATEGORY, DELETE_TASK, LOAD_CATEGORY, LOAD_TASK, TASK, TOGGLE_MODAL_CAT } from '..';
...
const TaskContextProvider = ({ children }: { children: JSX.Element }) => {
    ...
    const filterByCat = (category: Category) => {
        const filterTasks = state.tasks.filter(value => value.category.id === category.id)
